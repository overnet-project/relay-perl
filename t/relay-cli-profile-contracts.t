...
subtest 'relay help exposes profile contract controls' => sub {
  ...
  like $result->{stdout}, qr/--profile-contract\b/mx,        'help exposes profile contract file option';
  like $result->{stdout}, qr/--profile-contract-policy\b/mx, 'help exposes profile contract policy option';
};
...
subtest 'relay validates profile contract CLI inputs before listening' => sub {
  ...
  my $missing = _run($^X, $relay_script, '--host', '127.0.0.1', '--port', '0', '--profile-contract', $missing_path,);
  ...
  my $invalid_json =
    _run($^X, $relay_script, '--host', '127.0.0.1', '--port', '0', '--profile-contract', $invalid_json_path,);
  isnt $invalid_json->{exit}, 0, 'invalid profile contract JSON fails';
  like $invalid_json->{stderr}, qr/invalid\ profile\ contract\ JSON/mx, 'invalid JSON failure is explicit';

  my $not_object_path = File::Spec->catfile($tempdir, 'array.json');
  _write_file($not_object_path, '[]');
  my $not_object =
    _run($^X, $relay_script, '--host', '127.0.0.1', '--port', '0', '--profile-contract', $not_object_path,);
  ...
  my $invalid_contract =
    _run($^X, $relay_script, '--host', '127.0.0.1', '--port', '0', '--profile-contract-policy', 'known',
  ...
  my $invalid_policy =
    _run($^X, $relay_script, '--host', '127.0.0.1', '--port', '0', '--profile-contract-policy', 'strict',);
