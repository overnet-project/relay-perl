...
sub _spawn_relay_process {
  ...
  my $pid    = open3(
    ...
    '127.0.0.1',                             '--port',
    $args{port},                             '--name',
    'Overnet Live Relay',                    '--description',
    'Live relay test process',               '--software',
    'https://example.invalid/overnet-relay', '--version',
    '0.1.0-test',                            '--max-negentropy-sessions',
    4, (defined $args{store_file} ? ('--store-file', $args{store_file}) : ()),
  );
...
subtest 'relay process persists live object state across restart and suppresses duplicate replay' => sub {
  ...
  eval {
    ...
    my @event_msgs = grep { $_->type eq 'EVENT' && (($_->subscription_id || '') eq 'persist-sub') } @received;
...
subtest 'relay backup script copies persisted relay state for restore' => sub {
  ...
  eval {
    ...
    my @event_msgs = grep { $_->type eq 'EVENT' && (($_->subscription_id || '') eq 'backup-sub') } @received;
