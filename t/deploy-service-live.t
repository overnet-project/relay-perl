...
subtest 'relay service wrapper writes health and logs' => sub {
  ...
  my $proc = _spawn_process(
    $^X,             $relay_service_script, '--host',       '127.0.0.1',
    '--port',        $port,                 '--store-file', $store_file,
...
subtest 'IRC service wrapper writes health logs and TLS material' => sub {
  ...
  my $proc = _spawn_process(
    ...
    '--tls',               '--tls-cert-chain-file', $tls_cert_chain_file, '--tls-private-key-file',
    $tls_private_key_file, '--health-file',         $health_file,         '--log-file',
    $log_file,
  );
