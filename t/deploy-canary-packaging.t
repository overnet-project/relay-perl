...
my $code_canary_sync_a_env =
  File::Spec->catfile($code_root, 'deploy', 'canary', 'relay-sync-a-to-b', 'overnet-relay-sync.env.example');
my $code_canary_sync_a_config =
  File::Spec->catfile($code_root, 'deploy', 'canary', 'relay-sync-a-to-b', 'relay-sync.json.example');
my $code_canary_sync_b_env =
  File::Spec->catfile($code_root, 'deploy', 'canary', 'relay-sync-b-to-a', 'overnet-relay-sync.env.example');
my $code_canary_sync_b_config =
  File::Spec->catfile($code_root, 'deploy', 'canary', 'relay-sync-b-to-a', 'relay-sync.json.example');

my $irc_canary_readme = File::Spec->catfile($irc_root, 'deploy', 'canary', 'README.md');
my $relay_a_env =
  File::Spec->catfile($irc_root, 'deploy', 'canary', 'relay-a', 'overnet-irc-authority-relay.env.example');
my $relay_b_env =
...
for my $doc (['code canary README', _slurp($code_canary_readme)], ['IRC canary README', _slurp($irc_canary_readme)],) {
  my ($label, $text) = @{$doc};
