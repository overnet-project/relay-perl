...
sub _authoritative_nip29_stream_name {
  my (%args) = @_;
  return join ':', 'irc.authority.nip29', $args{network}, $args{group_host}, $args{group_id};
}
...
sub _build_authoritative_auth_payload {
  my (%args) = @_;
  return encode_base64(JSON::encode_json(_build_authoritative_auth_event_hash(%args)), '',);
}
...
sub _build_authoritative_delegate_payload {
  my (%args) = @_;
  return encode_base64(JSON::encode_json(_build_authoritative_delegate_event_hash(%args)), '',);
}
...
sub _read_authenticate_payload {
  ...
  while (1) {
    ...
    like $line, qr/\A(?::[^ ]+\ )?AUTHENTICATE\ (.+)\z/mx, 'server emits an AUTHENTICATE challenge line';
...
sub _assert_private_message_emitted_matches_fixture {
  ...
  my @recipient_tags =
    grep { ref($_) eq 'ARRAY' && @{$_} >= 2 && $_->[0] eq 'p' } @{$data->{decrypted_rumor}{tags} || []};
...
sub _assert_opaque_private_message_metadata {
  ...
  my @recipient_tags = grep { ref($_) eq 'ARRAY' && @{$_} >= 2 && $_->[0] eq 'p' } @{$data->{transport}{tags} || []};
...

{
  ...
  sub derive {
    ...
    my $channel_modes = '+'
      . join('',
      grep {$_} ($state->{closed} ? 'i' : ''),
      ($state->{moderated} ? 'm' : ''),
      'n', ($state->{topic_restricted} ? 't' : ''),
      );
    ...
    if ($operation eq 'authoritative_channel_view') {
      ...
      } grep { exists $state->{members}{$_} } sort keys %{$state->{present} || {}};
      my $admission = {
        allowed => JSON::false,
        member  => JSON::false,
    ...
    if ($operation eq 'authoritative_mode_write_permission') {
      ...
      my %permission = (
        ...
        object_type       => 'chat.channel',
        object_id         => 'irc:' . ($session->{network} || 'irc.test') . ':' . $channel,
        allowed           => $state->{tombstoned} ? JSON::false
        : $roles{'irc.operator'} ? JSON::true
        : JSON::false,
        mode => $mode,
        reason => $state->{tombstoned} ? 'deleted'
        : $roles{'irc.operator'} ? ''
        :                          'not_operator',
      );
      if (!$state->{tombstoned} && $roles{'irc.operator'}) {
        if ($mode =~ /\A[+-][ov]\z/mx && defined($mode_args->[0])) {
          ...
          $permission{current_roles} =
            ref($target_member) eq 'HASH'
    ...
    if ($operation eq 'authoritative_channel_action_permission') {
      ...
      if ($action eq 'undelete') {
        $permission{reason} =
           !$state->{tombstoned}   ? 'not_deleted'
          : $roles{'irc.operator'} ? ''
          :                          'not_operator';
        $permission{allowed} =
          ($state->{tombstoned} && $roles{'irc.operator'})
          ? JSON::true
          : JSON::false;
      } else {
        $permission{reason} =
            $state->{tombstoned}   ? 'deleted'
          : $roles{'irc.operator'} ? ''
          :                          'not_operator';
        $permission{allowed} =
          (!$state->{tombstoned} && $roles{'irc.operator'})
...
subtest 'IRC server program enforces nick uniqueness and emits 433 for collisions' => sub {
  ...
  my $host = Overnet::Program::Host->new(
    ...
    permissions => [
      'adapters.use',                 'subscriptions.read',
  ...
  $host->start;
  is $host->current_state, 'ready', 'nick-collision server reaches ready state';
  ...
  ok _wait_for_dm_subscription_count($host, 1), 'alice registration completes the first DM subscription open';
  ...
  ok _wait_for_dm_subscription_count($host, 2), 'bob registration completes the second DM subscription open';
  ...
  ok _wait_for_dm_subscription_count($host, 3), 'carol registration completes its DM subscription open';
  ...
  like $@, qr/IRC\ client\ disconnected\ before\ sending\ a\ line/mx, 'server closes the client connection after QUIT';
  ...
  ok _wait_for_dm_subscription_count($host, 5), 'dave registration completes its DM subscription open';

  _write_client_line($erin, 'NICK erin');
  close $erin->{socket};
  ok $host->pump(timeout_ms => 200) >= 0, 'server continues running after an unregistered client disconnects';
  ...
  ok _wait_for_dm_subscription_count($host, 6), 'frank registration completes its DM subscription open';
...
subtest 'IRC server program supports a minimal IRC client compatibility slice' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'compatibility server reaches ready state';
  ...
  is _read_client_line_optional($alice, 200), undef, 'CAP END does not emit any compatibility reply';
  ...
  is _read_client_line($alice, 1_000), ':overnet.irc.local 431 * :No nickname given', 'bare NICK returns 431';

  _write_client_line($alice, 'USER alice 0 *');
  is _read_client_line($alice, 1_000), ':overnet.irc.local 461 * USER :Not enough parameters', 'short USER returns 461';
...
subtest 'IRC server program accepts clients, emits Overnet output, and fans channel items back out' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'program reaches ready state under Host supervision';
  ...
  is _read_client_line_optional($bob, 200), undef, 'bob does not receive channel renders before joining the channel';
  ...
  is _read_client_line_optional($alice, 200), undef, 'alice no longer receives renders after parting the channel';
  ...
  _assert_signed_emitted_matches_fixture(
    $message_item, $privmsg->{expected}{event},
    $key,          'mapped channel PRIVMSG event',
    $privmsg_window,
  );
  ...
  _assert_signed_emitted_matches_fixture(
    $topic_item,                  $topic->{expected}{event}, $key,
    'mapped channel TOPIC state', $topic_window,             $topic_expected_content,
  );
  ...
  _assert_signed_emitted_matches_fixture($nick_item, $nick->{expected}{event},
    $key, 'mapped network NICK event', $nick_window,);
  ...
  _assert_signed_emitted_matches_fixture(
    $quit_item,   $quit->{expected}{event},
    $key,         'mapped channel QUIT event',
    $quit_window, $quit_expected_content,
  );

  my $transcript = $host->transcript;
  is _request_count_matching($transcript, 'from_program', 'subscriptions.open',
    sub { (($_[0]{subscription_id} || '') =~ /\Achannel:/mx) ? 1 : 0 },
    ),
    1, 'program opens one shared channel subscription';
  is _request_count_matching($transcript, 'from_program', 'subscriptions.close',
    sub { (($_[0]{subscription_id} || '') =~ /\Achannel:/mx) ? 1 : 0 },
    ),
...
subtest 'IRC server program routes direct messages through directional chat.dm objects' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'direct-message server reaches ready state';
  ...
  ok _wait_for_dm_subscription_count($host, 1), 'alice DM subscription opens after registration';
  ...
  ok _wait_for_dm_subscription_count($host, 2), 'program opens one DM subscription per registered nick';
  ...
  is _read_client_line_optional($alice, 200), undef, 'sender does not receive a synthetic DM echo';
  ...
  is _read_client_line_optional($bob, 200), undef, 'NOTICE sender does not receive a synthetic DM echo';
  ...
  is _request_count_matching($host->transcript, 'from_program', 'subscriptions.open',
    sub { (($_[0]{subscription_id} || '') =~ /\Adm:/mx) ? 1 : 0 },
    ),
...
subtest 'IRC server program blind-routes endpoint-blind E2E direct messages for E2EE-aware clients' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'E2EE direct-message server reaches ready state';
  ...
  ok _wait_for_dm_subscription_count($host, 1), 'alice registration completes the first E2EE DM subscription open';
  ...
  ok _wait_for_dm_subscription_count($host, 2), 'bob registration completes the second E2EE DM subscription open';
...
subtest 'IRC server program accepts TLS clients using the baseline tls config shape' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'TLS-enabled server reaches ready state';
  ...
  ok _wait_for_dm_subscription_count($host, 1), 'TLS client registration completes its DM subscription open';
  ...
  cmp_ok $message_item->{data}{created_at}, '<=', $time_window->{max},
    'TLS message created_at stays within the test window';
...
subtest 'IRC server program drops TLS handshakes on the plain listener without exiting' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'plain listener reaches ready state before the TLS probe';
  my $ready_details = _wait_for_ready_details($host);
...
subtest 'IRC server program uses authoritative hosted-channel state for moderated IRC behavior' => sub {
  ...
  my $host = Overnet::Program::Host->new(
    ...
    program_id  => 'overnet.program.irc_server',
    permissions => [
      'adapters.use',       'events.read',
      'subscriptions.read', 'overnet.emit_event',
  ...
  $host->start;
  is $host->current_state, 'ready', 'authoritative server reaches ready state';
  ...
  ok _wait_for_dm_subscription_count($host, 2), 'bob registration completes its DM subscription open';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps alice join requests';
  is _read_client_line($alice, 1_000), ":alice JOIN $channel", 'alice receives her JOIN echo';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps bob join requests';
  is _read_client_line($alice, 1_000), ":bob JOIN $channel", 'alice sees bob join the authoritative channel';
  is _read_client_line($bob,   1_000), ":bob JOIN $channel", 'bob receives his JOIN echo';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps MODE query requests';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps moderated PRIVMSG checks';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps topic privilege checks';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps TOPIC writes';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps TOPIC query requests';
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps MODE writes';
  is _read_client_line($alice, 1_000), ":alice MODE $channel +v bob",
    'operator mode changes are broadcast to the actor';
  is _read_client_line($bob, 1_000), ":alice MODE $channel +v bob", 'operator mode changes are broadcast to the target';

  _write_client_line($bob, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps NAMES derivation';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 353 bob = $channel :\@alice +bob",
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps KICK writes';
  is _read_client_line($alice, 1_000), ":alice KICK $channel bob :bye",
    'authoritative KICK is broadcast to the operator';
  is _read_client_line($bob, 1_000), ":alice KICK $channel bob :bye", 'authoritative KICK is broadcast to the target';

  _write_client_line($alice, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative host pumps post-KICK NAMES derivation';
  is _read_client_line($alice, 1_000), ":overnet.irc.local 353 alice = $channel :\@alice",
  ...
  ok $topic_request->{input}{group_metadata}{topic_restricted},
    'authoritative TOPIC preserves the topic-restricted metadata flag';
...
subtest 'IRC server program authenticates authoritative clients through SASL NOSTR' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'authoritative SASL server reaches ready state';
...
qr/\A:\Q$server_name\E\ CAP\ \*\ LS\ :message-tags\ server-time\ overnet-e2ee\ account-tag\ account-notify\ sasl\z/mx,
  ...
  is _read_client_line($alice, 1_000), ":$server_name CAP * ACK :sasl", 'authoritative SASL server ACKs CAP REQ :sasl';
  ...
  _write_client_line($alice, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative SASL host pumps alice join requests';
  is _read_client_line($alice, 1_000), ":alice JOIN $channel", 'alice receives her JOIN echo after SASL authentication';
  is _read_client_line($alice, 1_000), ":overnet.irc.local 353 alice = $channel :\@alice",
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'authoritative SASL host pumps TOPIC writes';
...
subtest 'IRC server program uses the real IRC adapter for authoritative NIP-29 channel state' => sub {
  ...
  my $host = Overnet::Program::Host->new(
    ...
    permissions => [
      ...
      'events.read',                  'subscriptions.read',
      'overnet.emit_event',           'overnet.emit_state',
      'overnet.emit_private_message', 'overnet.emit_capabilities',
    ],
    services => {
  ...
  $host->start;
  is $host->current_state, 'ready', 'real authoritative server reaches ready state';
  ...
  _write_client_line($alice, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps alice join requests';
  ...
  _write_client_line($bob, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps bob join requests';
  is _read_client_line($alice, 1_000), ":bob JOIN $channel",
    'alice sees bob join the authoritative channel on the real adapter path';
  is _read_client_line($bob, 1_000), ":bob JOIN $channel", 'bob receives his JOIN echo on the real adapter path';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 353 bob = $channel :\@alice bob",
  ...
  _write_client_line($bob, "MODE $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps MODE query requests';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 324 bob $channel +mnt",
    'real authoritative MODE query reflects derived channel modes';

  _write_client_line($bob, "PRIVMSG $channel :blocked");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps moderated PRIVMSG checks';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 404 bob $channel :Cannot send to channel",
  ...
  _write_client_line($bob, "TOPIC $channel :blocked");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps topic privilege checks';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 482 bob $channel :You're not channel operator",
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps TOPIC writes';
  ...
  _write_client_line($bob, "TOPIC $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps TOPIC query requests';
  ...
  _write_client_line($alice, "MODE $channel +v bob");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps MODE writes';
  is _read_client_line($alice, 1_000), ":alice MODE $channel +v bob",
  ...
  _write_client_line($bob, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps NAMES derivation';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 353 bob = $channel :\@alice +bob",
  ...
  _write_client_line($alice, "KICK $channel bob :bye");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps KICK writes';
  is _read_client_line($alice, 1_000), ":alice KICK $channel bob :bye",
  ...
  _write_client_line($alice, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'real authoritative host pumps post-KICK NAMES derivation';
  is _read_client_line($alice, 1_000), ":overnet.irc.local 353 alice = $channel :\@alice",
...
subtest 'IRC server program rejects non-member JOIN on a closed authoritative channel' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'closed authoritative server reaches ready state';
  ...
  _write_client_line($alice, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'closed authoritative host pumps alice join requests';
  is _read_client_line($alice, 1_000), ":alice JOIN $channel",
  ...
  _write_client_line($bob, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'closed authoritative host pumps bob join rejection';
  ...
  _write_client_line($alice, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'closed authoritative host pumps post-rejection NAMES';
  is _read_client_line($alice, 1_000), ":overnet.irc.local 353 alice = $channel :\@alice",
...
subtest 'IRC server program admits an invited user to a closed authoritative channel' => sub {
  ...
  $host->start;
  is $host->current_state, 'ready', 'invite authoritative server reaches ready state';
  ...
  _write_client_line($alice, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps alice join requests';
  is _read_client_line($alice, 1_000), ":alice JOIN $channel",
  ...
  _write_client_line($bob, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps bob pre-invite join rejection';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 473 bob $channel :Cannot join channel (+i)",
    'closed authoritative channel rejects bob before invite';

  _write_client_line($alice, "INVITE bob $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps INVITE writes';
  ...
  is _read_client_line($bob, 1_000), ":alice INVITE bob :$channel", 'target receives the INVITE line';
  ...
  _write_client_line($bob, "JOIN $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps bob invited join';
  is _read_client_line($alice, 1_000), ":bob JOIN $channel",
    'alice sees the invited user join the closed authoritative channel';
  is _read_client_line($bob, 1_000), ":bob JOIN $channel", 'bob receives his JOIN echo after invite';
  is _read_client_line($bob, 1_000), ":overnet.irc.local 353 bob = $channel :\@alice bob",
  ...
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps carol uninvited join rejection';
  ...
  _write_client_line($alice, "NAMES $channel");
  ok $host->pump(timeout_ms => 200) >= 0, 'invite authoritative host pumps post-invite NAMES';
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program establishes authoritative relay delegation through SASL NOSTR' => sub {
    ...
    $host->start;
    is $host->current_state, 'ready', 'authoritative SASL relay server reaches ready state';
...
qr/\A:\Q$server_name\E\ CAP\ \*\ LS\ :message-tags\ server-time\ overnet-e2ee\ account-tag\ account-notify\ sasl\z/mx,
    ...
    is $challenge_payload->{scope},
      _authoritative_auth_scope(
      server_name => $server_name,
      network     => $network,
      ),
      'relay-backed SASL challenge carries the authoritative auth scope';
    is $challenge_payload->{relay_url}, $relay_url, 'relay-backed SASL challenge carries the relay URL';
    ...
    like $challenge_payload->{expires_at}, qr/\A\d+\z/mx, 'relay-backed SASL challenge carries a delegation expiration';
    ...
    _write_client_line($alice, "JOIN $channel");
    ok $host->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'authoritative SASL relay host pumps alice join requests';
    ...
    ok $host->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'authoritative SASL relay host pumps TOPIC writes';
    ok _pump_hosts_until(
      hosts           => [$host],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program relay-publishes authoritative NIP-29 writes across two instances' => sub {
    ...
    my $build_runtime = sub {
      ...
      $host->start;
      is $host->current_state, 'ready', "$args{name} relay-backed authoritative server reaches ready state";
...
qr/\A:\Q$args{server_name}\E\ NOTICE\ \Q$args{nick}\E\ :OVERNETAUTH\ DELEGATE\ ([0-9a-f]{64})\ ([0-9a-f]{64})\ \Q$relay_url\E\ (\d+)\z/mx,
    ...
    ok _wait_for_dm_subscription_count($host_a, 1), 'instance A alice registration completes its DM subscription open';
    ...
    ok _wait_for_dm_subscription_count($host_a, 2), 'instance A bob registration completes its DM subscription open';
    ...
    ok _wait_for_dm_subscription_count($host_b, 1), 'instance B bob registration completes its DM subscription open';
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps alice relay-backed join request';
    ...
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps bob pre-invite join request';
    ok _pump_hosts_until(
      hosts           => [$host_a, $host_b],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps relay-backed TOPIC write';
    ok _pump_hosts_until(
      hosts           => [$host_a, $host_b],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
        ...
        condition       => sub {
          ...
          return scalar(
            grep {
              ...
              defined($tags{topic})
                && $tags{topic} eq 'Relay-backed topic'
                && defined($tags{overnet_actor})
                && $tags{overnet_actor} eq $alice_pubkey
                && defined($tags{overnet_authority})
                && $tags{overnet_authority} =~ /\A[0-9a-f]{64}\z/mx
                && defined($tags{overnet_sequence})
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps relay-backed INVITE write';
    ...
    _write_client_line($bob_b, "JOIN $channel");
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps bob relay-backed invited join';
    ...
    ok $relay_join_request,
      'instance B routes the relay-backed JOIN through the adapter when invite admission is available';
    ...
    ok $bob_b_join_echo,
      ...
      or diag(
      'instance B post-join lines: ' . (@bob_b_post_join_lines ? join(' | ', @bob_b_post_join_lines) : '(none)'),
      'instance B last subscriptions.open request: '
        . JSON::encode_json(_last_request_matching($host_b->transcript, 'from_program', 'subscriptions.open',) || {}),
      'instance B last nostr.publish_event request: '
        . JSON::encode_json(_last_request_matching($host_b->transcript, 'from_program', 'nostr.publish_event',) || {}),
      );
    ...
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps propagated authoritative TOPIC query';
    ...
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps bob relay-backed PART';
    ...
    ok(
      scalar(
        grep {
          my %tags = _first_tag_values($_->{tags});
          defined($tags{overnet_actor})
            && $tags{overnet_actor} eq $bob_pubkey
            && defined($tags{overnet_authority})
            && $tags{overnet_authority} =~ /\A[0-9a-f]{64}\z/mx
            && defined($tags{overnet_sequence})
            && $tags{overnet_sequence} =~ /\A[1-9]\d*\z/mx
    ...
    ok _pump_hosts_until(
      ...
      or diag(
      'instance A post-PART lines: ' . (@alice_a_post_part_lines ? join(' | ', @alice_a_post_part_lines) : '(none)'),
      );

    _write_client_line($bob_b, "JOIN $channel");
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps bob post-PART rejoin attempt';
    ...
    my %relay_invite_ids_before_reinvite =
      map { defined($_->{id}) && !ref($_->{id}) ? ($_->{id} => 1) : () } @{$relay_invites_before_reinvite || []};
    ...
    _write_client_line($alice_a, "INVITE bob $channel");
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps a fresh relay-backed INVITE after PART';
    ...
    ok _pump_hosts_until(
      ...
      condition       => sub {
        ...
        return $invite_request_count_after_reinvite > $invite_request_count_before_reinvite
          && $invite_publish_count_after_reinvite > $invite_publish_count_before_reinvite ? 1 : 0;
      },
      ),
    ...
    ok(
      ...
      'instance A fresh INVITE request count: '
        . $invite_request_count_before_reinvite . ' -> '
      ...
      'instance A fresh INVITE publish count: '
        . $invite_publish_count_before_reinvite . ' -> '
        . _request_count_matching(
        $host_a->transcript,
        'from_program',
        'nostr.publish_event',
      ...
      'instance A last authoritative derive request: '
        . JSON::encode_json(_last_request_matching($host_a->transcript, 'from_program', 'adapters.derive',) || {}),
      'instance A last subscription snapshot request: '
        . JSON::encode_json(
        _last_request_matching($host_a->transcript, 'from_program', 'nostr.read_subscription_snapshot',) || {}
        ),
      'instance A last fresh INVITE map_input request: ' . JSON::encode_json(
    ...
    ok $bob_b_reinvite_join_echo, 'bob receives his JOIN echo after the fresh relay-backed INVITE'
      or
      diag('instance B fresh-invite lines: ' . (@bob_b_reinvite_lines ? join(' | ', @bob_b_reinvite_lines) : '(none)'),
      );
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps relay-backed MODE write';
    ok _pump_hosts_until(
      hosts           => [$host_a, $host_b],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
      condition       => sub {
        ...
        return $mode_request_count_after > $mode_request_count_before
          && $mode_publish_count_after > $mode_publish_count_before ? 1 : 0;
      },
      ),
    ...
    ok _pump_hosts_until(
      ...
      or diag(
      'instance A post-MODE lines: ' . (@alice_a_mode_lines ? join(' | ', @alice_a_mode_lines) : '(none)'),
      );
...
'instance B authoritative NAMES reflects bob voice alongside retained remote presence after the relay-backed MODE';
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps relay-backed KICK write';
    ok _pump_hosts_until(
      hosts           => [$host_a, $host_b],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
    ...
    is $shutdown_a->{state}, 'shutdown_complete',
      'instance A relay-backed authoritative server handles runtime shutdown';
    ...
    is $shutdown_b->{state}, 'shutdown_complete',
      'instance B relay-backed authoritative server handles runtime shutdown';
    is $shutdown_b->{exit_code}, 0, 'instance B relay-backed authoritative server exits cleanly';

    ok _request_count_matching($host_a->transcript, 'from_program', 'nostr.open_subscription',
      sub { (($_[0]{relay_url} || '') eq $relay_url) ? 1 : 0 },
    ) >= 1, 'instance A opens a runtime nostr subscription for authoritative relay state';
    ok _request_count_matching($host_b->transcript, 'from_program', 'nostr.open_subscription',
      sub { (($_[0]{relay_url} || '') eq $relay_url) ? 1 : 0 },
    ) >= 1, 'instance B opens a runtime nostr subscription for authoritative relay state';
    ok _request_count_matching($host_a->transcript, 'from_program', 'nostr.read_subscription_snapshot', sub {1},) >= 1,
      'instance A reads runtime nostr subscription snapshots';
    ok _request_count_matching($host_b->transcript, 'from_program', 'nostr.read_subscription_snapshot', sub {1},) >= 1,
      'instance B reads runtime nostr subscription snapshots';
    ok _request_count_matching(
      $host_a->transcript,
      'from_program',
      'nostr.publish_event',
      sub {
        ref($_[0]{event}) eq 'HASH'
          && (($_[0]{event}{kind} || 0) == 14142
          || ($_[0]{event}{kind} || 0) == 9009
          || ($_[0]{event}{kind} || 0) == 9002
          || ($_[0]{event}{kind} || 0) == 9001);
      },
    ...
    ok _request_count_matching($host_a->transcript, 'from_program', 'adapters.derive',
    ...
    ok _request_count_matching($host_b->transcript, 'from_program', 'adapters.derive',
      sub { (($_[0]{operation} || '') eq 'authoritative_join_admission') ? 1 : 0 },
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program creates and discovers authoritative hosted channels across two instances' => sub {
    ...
    my $group_id = join('-', 'irc', unpack('H*', $network), unpack('H*', $canonical_channel),);
    ...
    my $build_runtime = sub {
      ...
      $host->start;
      is $host->current_state, 'ready', "$args{name} created-channel server reaches ready state";
      my $ready = _wait_for_ready_details($host);
...
qr/\A:\Q$args{server_name}\E\ NOTICE\ \Q$args{nick}\E\ :OVERNETAUTH\ DELEGATE\ ([0-9a-f]{64})\ ([0-9a-f]{64})\ \Q$relay_url\E\ (\d+)\z/mx,
    ...
    _write_client_line($alice_a, "JOIN $channel");
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps hosted-channel creation JOIN';
    ...
    ok(
      _pump_hosts_until(
        ...
        condition       => sub {
          ...
          my $has_operator = scalar(
            grep {
              my %tags = _first_tag_values($_->{tags});
              defined($tags{p})
                && $tags{p} eq $alice_pubkey
                && defined($tags{overnet_actor})
                && $tags{overnet_actor} eq $alice_pubkey
            } @{$role_events}
          ) ? 1 : 0;
    ...
    ok $discovered_list_lines, 'instance B emits the LIST response for the discovered hosted channel';
    ...
    ok $discovered_join_request, 'instance B routes the discovered hosted-channel JOIN through the adapter';
    ...
    ok $discovered_join_publish, 'instance B publishes the discovered hosted-channel JOIN to the relay';
    ...
    ok $bob_part_request, 'instance B routes the discovered hosted-channel PART through the adapter';
    ...
    ok $bob_part_publish, 'instance B publishes the discovered hosted-channel PART to the relay';
    my %bob_part_publish_tags = _first_tag_values($bob_part_publish->{event}{tags});
    is $bob_part_publish_tags{h}, $group_id, 'the discovered hosted-channel PART publish targets the expected group id';
    ...
    ok $empty_list_lines, 'instance B emits the LIST response after the created hosted channel empties';
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program tombstones authoritative hosted channels across two instances' => sub {
    ...
    my $build_runtime = sub {
      ...
      $host->start;
      is $host->current_state, 'ready', "$args{name} tombstone server reaches ready state";
      my $ready = _wait_for_ready_details($host);
...
qr/\A:\Q$args{server_name}\E\ NOTICE\ \Q$args{nick}\E\ :OVERNETAUTH\ DELEGATE\ ([0-9a-f]{64})\ ([0-9a-f]{64})\ \Q$relay_url\E\ (\d+)\z/mx,
    ...
    ok $delete_request, 'instance A routes the authoritative hosted-channel delete through the adapter';
    ok _request_count_matching(
      $host_a->transcript,
      'from_program',
      'adapters.derive',
    ...
    ok $delete_publish, 'instance A publishes a tombstoning metadata edit to the relay';
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program reactivates tombstoned authoritative hosted channels across two instances' => sub {
    ...
    my $build_runtime = sub {
      ...
      $host->start;
      is $host->current_state, 'ready', "$args{name} undelete server reaches ready state";
      my $ready = _wait_for_ready_details($host);
...
qr/\A:\Q$args{server_name}\E\ NOTICE\ \Q$args{nick}\E\ :OVERNETAUTH\ DELEGATE\ ([0-9a-f]{64})\ ([0-9a-f]{64})\ \Q$relay_url\E\ (\d+)\z/mx,
    ...
    ok $undelete_request, 'instance A routes the authoritative hosted-channel UNDELETE through the adapter';
    ok _request_count_matching(
      $host_a->transcript,
      'from_program',
      'adapters.derive',
    ...
    my $undelete_publish = _last_request_matching(
      ...
      sub {
        ...
        return
          scalar
          grep { ref($_) eq 'ARRAY' && @{$_} >= 2 && (($_->[0] || '') eq 'topic') && (($_->[1] || '') eq $topic_text) }
          @tags;
      },
    );
    ok $undelete_publish, 'instance A publishes an undelete metadata edit that retains the prior topic';
    ...
    ok _pump_hosts_until(
      ...
      condition       => sub {
        ...
        my @sorted = sort { (($a->{created_at} || 0) <=> ($b->{created_at} || 0)) } @{$metadata_events};
        my $latest = $sorted[-1];
    ...
    ok $undeleted_list_lines, 'instance B emits a LIST response after the hosted channel is undeleted';
    ...
    is(
      ...
      [":$server_name_a 353 alice = $channel :bob", ":$server_name_a 366 alice $channel :End of /NAMES list.",],
...
if (_run_program_irc_server_group('relay')) {
  subtest 'IRC server program enforces authoritative bans across two instances' => sub {
    ...
    my $build_runtime = sub {
      ...
      $host->start;
      is $host->current_state, 'ready', "$args{name} authoritative ban server reaches ready state";
      my $ready = _wait_for_ready_details($host);
...
qr/\A:\Q$args{server_name}\E\ NOTICE\ \Q$args{nick}\E\ :OVERNETAUTH\ DELEGATE\ ([0-9a-f]{64})\ ([0-9a-f]{64})\ \Q$relay_url\E\ (\d+)\z/mx,
    ...
    _write_client_line($alice_a, "JOIN $channel");
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps the operator join before setting bans';
    ...
    ok $host_a->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance A pumps authoritative MODE +b';
    ok _pump_hosts_until(
      hosts           => [$host_a, $host_b],
      pump_timeout_ms => $relay_host_pump_ms,
      timeout_ms      => $relay_propagation_timeout_ms,
    ...
    ok(
      scalar(
        grep {
          my %tags     = _first_tag_values($_->{tags});
          my @ban_tags = map { $_->[1] }
            grep { ref($_) eq 'ARRAY' && (($_->[0] || '') eq 'ban') && defined $_->[1] } @{$_->{tags} || []};
          defined($tags{overnet_actor})
            && $tags{overnet_actor} eq $alice_pubkey
    ...
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps the authoritative ban-list query';
    ...
    _write_client_line($bob_b, "JOIN $channel");
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps the banned rejoin attempt';
    ...
    ok !defined($propagated_part_before_unban_query)
      || $propagated_part_before_unban_query eq ":bob PART $channel :bye",
    ...
    ok !
      scalar(grep { ref($_) eq 'ARRAY' && (($_->[0] || '') eq 'ban') && defined($_->[1]) && $_->[1] eq $bob_mask }
        @{$latest_unban_event->{tags} || []}),
      'the latest authoritative metadata edit no longer carries the removed ban mask';

    _write_client_line($bob_b, "JOIN $channel");
    ok $host_b->pump(timeout_ms => $relay_host_pump_ms) >= 0, 'instance B pumps the post-unban rejoin';
...
subtest 'IRC program entrypoints do not import Net::Nostr directly' => sub {
  ...
  for my $path (@paths) {
    ...
    unlike $source, qr{(?:\A|\n)\s*use\s+Net::Nostr}xm, "$path no longer imports Net::Nostr directly";
    unlike $source, qr/\bNet::Nostr::/mx,               "$path no longer references Net::Nostr directly";
  }
};
