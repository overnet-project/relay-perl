...
for my $doc (['README', $readme_text], ['AGENTS', $agents_text],) {
  my ($label, $text) = @{$doc};
  like $text, qr/IRC\ verification\ path/imx, "$label documents the IRC verification path";
  like $text, qr/t\/spec-conformance-irc-server\.t/mx,
    "$label includes server conformance in the IRC verification path";
  like $text, qr/t\/program-irc-server\.t/mx, "$label includes the fast IRC server suite in the IRC verification path";
  ...
  like $text, qr/t\/relay-sync-live\.t/mx, "$label includes live relay sync coverage in the IRC verification path";
  like $text, qr/t\/deploy-restore-drill-live\.t/mx, "$label includes the restore drill in the IRC verification path";
}
