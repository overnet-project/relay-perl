...
subtest 'relay sync CLI loads static config and syncs one peer into the local relay' => sub {
  ...
  eval {
    ...
    my $stderr = gensym();
    my $pid    = open3(my $stdin, my $stdout, $stderr, $^X, $sync_script, '--config', $config_path,);
    close $stdin;
