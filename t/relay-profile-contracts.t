...
subtest 'configured contracts default to known policy and validate matching events only' => sub {
  ...
  ok _publish($relay, $unknown)->accepted,
    'known policy accepts events whose event type is not configured';
...
subtest 'off policy keeps configured contracts non-enforcing' => sub {
  ...
  ok _publish($relay, $invalid)->accepted,
    'off policy accepts core-valid events even when contract would reject them';
...
subtest 'required policy rejects profile events without a configured contract' => sub {
  ...
  ok _publish($relay, $delegation)->accepted,
    'required policy leaves defined core protocol events to core validation';
};
